...
SLICAP::SLICAP(h_Mat source): 
	h_labels(source.size(), CV_32SC1),
	SLIC_result(source.size(), CV_8UC3),
	AP_result(source.size(), CV_8UC3)
{
    std::cout << "constructing SLICAP" << std::endl;
    h_source = source;

    std::cout << "initializing SLIC_eng" << std::endl;

    int num_pixels = source.cols * source.rows;
    int space_between_centers = sqrt(source.cols * source.rows) / Param::superpixel_size_factor;
    int density_modifier = Param::density / space_between_centers;
    int SP_rows = floor(source.rows/space_between_centers);
    int SP_cols = floor(source.cols/space_between_centers);
    int num_superpixels=SP_rows * SP_cols;

    SLIC_eng = new SLIC(source.cols, source.rows, num_pixels, space_between_centers, density_modifier, SP_rows, SP_cols, num_superpixels, this);

    std::cout << "initializing AP_eng" << std::endl;
    AP_eng = new AP(this);

    std::cout << "uploading source and labels" << std::endl;
    d_source.upload(h_source);
    d_labels.upload(h_labels);

    std::cout << "splitting source into CIELAB planes" << std::endl;
    h_Mat CIELAB_src;
...
void SLICAP::run_SLIC(){ 
    ...
    } while (SLIC_eng->displacement > Param::displacement_threshold);
...
void SLICAP::run_AP(){ 
    ...
    while (AP_eng->constant_cycles < Param::num_constant_cycles_for_convergence) {
